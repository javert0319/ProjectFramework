...
public class ToolbarManager {
     ...
     * @param onKeyboardListener keyboard监听
     * @return
     */
    public Toolbar initToolbar(Toolbar toolbar, String title, boolean displayHomeEnable, OnKeyboardListener onKeyboardListener, Boolean keyboardEnable) {
        if (toolbar != null) {
            Activity context = contextRef.get();
            if (context == null) return null;


            ImmersionBar immer = ImmersionBar.with(context).titleBar(toolbar);
            if (keyboardEnable) {
                immer.keyboardEnable(true);
            }
            immer.setOnKeyboardListener(onKeyboardListener);
            immer.init();
